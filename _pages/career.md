...
### - 프로젝트 요약
 ...
 - DynamoDB를 사용한 이유는 다음과 같았다. 
    - 대량의 dict 형태의 데이터를 put 하고 입맛대로 쿼리하기에는 NoSQL 이 편했다. 그리고 SQL 문 안 짜도 된다. 스키마리스... NoSQL 의 대표 장점이라고나 할까. 적어도 내가 쓰는 용도에는 장점이 됨.
    - 여러 NoSQL 이 있지만 DynamoDB 를 쓴 진짜 이유는!
    - DynamoDB의 데이터를 Lambda 에서만 사용하게 되는데, RDS 를 이용해야 할 경우 PyMySQL 를 사용해야 되지만 이건 그렇지 않아도 됨. boto3 는 빌트-인. 라이브러리 추가 안 해도 돼서 콜드 스타트에 조금의 이점도 생김. (module 을 layers 로 분리 시켜도 Lambda 의 container 에는 같이 포함되기 때문에 cold start 가 줄진 않음). 조금이라도 cold start 를 줄이고 싶었던 나의 욕망.
    - 그리고 거의 공짜로 쓸 수 있었음. 내 경우엔 SI를 따로 만든 것도 없고 LCU 가 작아서 그런지 월마다 0.25달러 정도의 요금밖에 안 나옴.
...
유료로 사용하던 비용을 절감하는 성과는 거둠. 그러나 너무 똑같이 만들었다. 

벤치마킹한 프로젝트도 앞으로의 비즈니스 요구사항을 소화할 수 없는 미완성 프로젝트.

랜딩 페이지의 다양한 폼을 수용할 수 없었음. 

### - 프로젝트 요약
  - 웹 프레임워크로 Laravel 을 사용. 사용한 이유는 'PHP 가 생산성이 좋아서'. PHP 를 쓰긴 쓸 건데 웹 프레임워크로는 무엇을 만들어 볼까 고민하다 찾은게 Laravel.
  - 기관에서 배운 Java 지식으로는 오랫 동안 안 해서 많이 까먹기도 했고 다시 공부해서 해야할 프로젝트를 소화하기가 부담이 됨.
  - 한참 뒤에 깨달은 거지만, 그냥 기관에서 배운 Spring 지식 좀 더 공부해서 만들 껄 하는 적잖은 후회가 밀려옴. 어떤 언어든 깊이는 있고 제대로 쓸 줄 알아야 생산성이 좋은 것 이다. 
  - Laravel document 열심히 보며 MVC 패턴에 맞게 기획한 데로 프로젝트를 하나하나 빌딩해 나감.
  - 카페24에서 구매한 서버에 nginx, php-fpm 을 설치하고, 개발한 프로젝트를 배포하였고, 잘 작동해 주었음. 이 또한, 구글링의 친절한 설명으로 잘 마무리 지음. 난생 처음 만든 상용 서비스.   
